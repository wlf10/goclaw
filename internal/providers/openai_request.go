...
func (p *OpenAIProvider) buildRequestBody(model string, req ChatRequest, stream bool) map[string]any {
	...
	for _, m := range inputMessages {
		...
		// Echo reasoning_content for models that require it on EVERY assistant
		// message — DeepSeek thinking mode returns HTTP 400 if any assistant
		// message lacks the field, even when empty. The allowlist in
		// openAIWireAssistantReasoningContent ensures we only emit for
		// compatible APIs (DeepSeek, Kimi, OpenAI o-series).
